...
export class Schedule_of_trains_service {
  ...
  async createPath(
    Create_schedule_of_train_dto: Create_schedule_of_train_dto,
  ): Promise<Schedule_of_trains_entity> {
    const {
      Arrival_time,
      Departure_time,
      Number_of_train,
      Path_of_train,
      Type_of_train,
    } = Create_schedule_of_train_dto;
    const path = new Schedule_of_trains_entity();
  ...
  async findOnePath(id: number): Promise<Schedule_of_trains_entity> {
    const path = await this.Schedule_of_trains_repository.findOne({
      where: { id },
    });
  ...
  async updatePath(
    id: number,
    Update_schedule_of_train_dto: Update_schedule_of_train_dto,
  ): Promise<Schedule_of_trains_entity> {
    const path_to_update = await this.Schedule_of_trains_repository.findOne({
      where: { id },
    });
    const updated_path = Object.assign(
      path_to_update,
      Update_schedule_of_train_dto,
    );
